...
def userRegister(request):
    ...
    #     if form.is_valid():
    #         form.save()
    #         messages.success(reques,"Başarıyla kayıt olundu.")
    ...
    if request.method == "POST":
        ...
        if sifre1 == sifre2:
            if User.objects.filter(email = email).exists():
                messages.error(request,"Bu mail kullanılıyor.")
                ...
                subject = "deneme yapıyorum"
                message = "deneme yaptım"
                send_mail(
                    subject,
                    message,
...
def passwordChange(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request,"Başarısızsın.")
    else:
...
def accountDelete(request,):
    if request.method == "POST":
        user = request.user
        user.delete()
        messages.success(request,"Hesabınız başarı ile silindi.")
        return redirect("register")
